...
# Import OMM-12 absolute abundance data and parameters
    ...
    for type in col_names[i-1]:
        ...
        ax.hist(logistic_posteriors.loc[:, f'dR_{j}'], color = col_comunities[i-1], alpha = 0.4, density=True, bins = 20, log=True)


        mean_value = logistic_posteriors.loc[:, f'dR_{j}'].mean()
...
for ax in [axB05, axB06, axB07, axB08, axB09]:
    ax.set_xlabel(r'death (cells/day)', fontsize=15)
...
#mp.tight_layout()
mp.savefig('./fig_dead.pdf', dpi=300, format='pdf', bbox_inches='tight')