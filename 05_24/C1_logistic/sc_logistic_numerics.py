...
def eq_moments_abs_abund_closure_from_3rd(t, y, gR, dR, N):    # Unpack different moments        
    m_k = y[:n_types]
...
## Moment Closure - 3rd moments (WORKING)
def eq_moments_rel_abund_closure_from_3rd(t, y, gR, dR, N):
...
def eq_moments_abs_abund_closure_from_2nd(t, y, gR, dR, N):
    # Unpack different moments
    m_k = y[:n_types]
...
## Moment Closure - 2nd & 3rd moments (WORKING)
def eq_moments_rel_abund_closure_from_2nd(t, y, gR, dR, N):
   # Unpack different moments         
    m_k = y[:n_types]
    m_k_prod = np.outer(m_k, m_k)
    m_Sigma = y[-1]
...
def stopping_event_abs_abund(t, y, gR, dR, N):
...
def model_abs_abund_closure_from_3rd(parameters):
    ...
    if sum((gR<0) + (dR<0) ) + (N<0) == 0:
        
        # Solve the model numerically
        predicted_moments = integrate.solve_ivp(eq_moments_abs_abund_closure_from_3rd, [0, t_simulated], init_abs_abund, args = (gR, dR, N), t_eval = sampling_times, method = 'LSODA', events=stopping_event_abs_abund)
...
def model_abs_abund_closure_from_2nd(parameters):
    ...
    if sum((gR<0) + (dR<0)) + (N<0) == 0:
        ...
        # Solve the model numerically
        predicted_moments = integrate.solve_ivp(eq_moments_abs_abund_closure_from_2nd, [0, t_simulated], init_abs_abund, args = (gR, dR, N), t_eval = sampling_times, method = 'LSODA', events=stopping_event_abs_abund)
...
# Stopping event for diverging numerical solutions
def stopping_event_rel_abund(t, y, gR, dR, N):
...
def model_rel_abund_closure_from_3rd(parameters):
    ...
    # Check that all parameters values are meaningful
    if sum((gR<0) + (dR<0)) + (N<0) + (m_Sigma<0) == 0:

        # Solve the model numerically
        predicted_moments = integrate.solve_ivp(eq_moments_rel_abund_closure_from_3rd, [0, t_simulated], np.hstack((init_rel_abund, m_Sigma)), args = (gR, dR, N), t_eval = sampling_times, method = 'LSODA', events=stopping_event_rel_abund)
...
def model_rel_abund_closure_from_2nd(parameters):
    ...
    if sum((gR<0) + (dR<0)) + (N<0) + (m_Sigma<0) == 0:
        ...
        # Solve the model numerically
        predicted_moments = integrate.solve_ivp(eq_moments_rel_abund_closure_from_2nd, [0, t_simulated], np.hstack((init_rel_abund, m_Sigma)), args = (gR, dR, N), t_eval = sampling_times, method = 'LSODA', events=stopping_event_rel_abund)
